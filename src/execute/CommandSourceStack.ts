...
export class CommandSourceStack {
    ...
    public runCommand(command: string): CommandResult {
        if (this.position instanceof Entity) {
            ...
            const commandString = `execute in ${this.dimension.id.replace("minecraft:", "")} rotated ${this.rotation.format("$yaw $pitch", 4)} positioned as @n[tag="${temporaryTag}"] run ${command.trim()}`;
