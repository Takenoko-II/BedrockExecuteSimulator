...
export class EntitySelectorParser extends AbstractParser<EntitySelector, EntitySelectorInterpretError> {
    ...
    private type(): SelectorType | undefined {
        for (const { identifier, value } of ENTITY_SELECTOR_REGISTRIES.get("bedrock_execute_simulator:entity_selector_types").getEntries()) {
            if (this.next(true, identifier.value)) {
                return value;
            }
        }
    ...
    private arguments(): SelectorArguments {
        ...
        const registry = ENTITY_SELECTOR_REGISTRIES.get("bedrock_execute_simulator:entity_selector_argument_types");
        const serializer = new Serializer();
        serializer.indentationSpaceCount = 1;
        serializer.linebreakable = false;

        for (const [name, list] of Object.entries(inputs)) {
            if (!registry.contains(id(name))) {
                throw new EntitySelectorInterpretError("不明なセレクタ引数です: '" + name + "'");
            }

            const argumentType = registry.get(id(name));
